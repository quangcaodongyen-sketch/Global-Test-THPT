...
import { getSchoolConfig, saveSchoolConfig } from '../utils/licenseManager';
...
export const InputForm: React.FC<InputFormProps> = ({
  ...
  const [grade, setGrade] = useState<Grade>('Lớp 10');
  const [examType, setExamType] = useState<ExamType>('Giữa kỳ 1');
  const [adminInfo, setAdminInfo] = useState<AdminInfo>(() => {
    const defaultInfo = DEFAULT_ADMIN_INFO['Lớp 10'];
    const cfg = getSchoolConfig();
    return {
      ...defaultInfo,
      schoolName: cfg.schoolName || 'THPT Đồng Yên',
    };
  });
  ...
  useEffect(() => {
    ...
    setAdminInfo((prev) => ({
      ...defaultInfo,
      schoolName: prev.schoolName || getSchoolConfig().schoolName || 'THPT Đồng Yên',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Tự động lưu tên trường vào tài khoản người dùng
    if (adminInfo.schoolName?.trim()) {
      saveSchoolConfig(getSchoolConfig().parentAgency, adminInfo.schoolName.trim());
    }
    onGenerateExam({
      grade,
      examType,
  ...
  const handleUseStandardMoetTemplate = () => {
    setUploadedTemplates([
      {
        name: 'Mau_TiengAnh_THPT_TuyenQuang.docx',
        type: 'matrix',
        content: 'Mẫu ma trận và bản đặc tả Tiếng Anh THPT Tuyên Quang',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            ...
            1. Chọn Khối Lớp (Global Success THPT)
          </label>
          <div className="grid grid-cols-3 gap-2">
            {(['Lớp 10', 'Lớp 11', 'Lớp 12'] as Grade[]).map((g) => (
              <button
                key={g}
                type="button"
                onClick={() => setGrade(g)}
          ...
          <div>
            ...
            <input
              ...
              onBlur={(e) => {
                ...
                if (val) {
                  saveSchoolConfig(getSchoolConfig().parentAgency, val);
                }
              }}
